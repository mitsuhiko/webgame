...
use crate::protocol::{Message, Packet, PlayerConnectedMessage, PlayerDisconnectedMessage};
...
impl Game {
    pub fn new(universe: Arc<Universe>) -> Game {
        Game {
            id: Uuid::new_v4(),
    ...
    pub async fn add_player(&self, player_id: Uuid) {
        ...
        if let Some(player_info) = universe.get_player_info(player_id).await {
            drop(game_state);
            self.broadcast(&Packet::Message(Message::PlayerConnected(
    ...
    pub async fn remove_player(&self, player_id: Uuid) {
        ...
        if game_state.players.remove(&player_id).is_some() {
            drop(game_state);
            self.broadcast(&Packet::Message(Message::PlayerDisconnected(
                PlayerDisconnectedMessage { player_id },
            )))
            .await;
        }
    }
    ...
    pub async fn mark_player_ready(&self, player_id: Uuid) {
        ...
        if game_state.players.values().all(|x| x.ready) {
            ...
            drop(game_state);
            self.broadcast(&Packet::Message(Message::PregameStarted))
                .await;
        }
    }
    ...
    pub async fn broadcast(&self, packet: &Packet) {
        ...
        for player_state in game_state.players.values() {
            universe.send(player_state.player_id, &packet).await;
        }
    }
}
